@component('layouts.admin', ['title' => 'Kelola Skill untuk ' . $subCompetition->name])
<div class="bg-white rounded-xl shadow-lg p-6 border border-gray-100">
    <div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            ...
            <div class="mt-3">
                <a href="{{ route('admin.competitions.sub-competitions.index', $competition->id) }}" class="inline-flex items-center text-sm text-blue-600 hover:text-blue-800 transition-colors duration-200">
...
@include('admin.competitions.sub-competitions.skills.components.add-skill-modal')
@include('admin.competitions.sub-competitions.skills.components.edit-skill-modal')
@include('admin.competitions.sub-competitions.skills.components.delete-skill-modal')

<!-- JavaScript Setup -->
<script>
    // Define routes for skills
    window.skillRoutes = {
        store: "{{ route('admin.competitions.sub-competitions.skills.store', ['competition' => $competition->id, 'sub_competition' => $subCompetition->id]) }}",
        update: "{{ route('admin.competitions.sub-competitions.skills.update', ['competition' => $competition->id, 'sub_competition' => $subCompetition->id, 'skill' => '__id__']) }}",
        destroy: "{{ route('admin.competitions.sub-competitions.skills.destroy', ['competition' => $competition->id, 'sub_competition' => $subCompetition->id, 'skill' => '__id__']) }}",
        available: "{{ route('admin.competitions.sub-competitions.skills.available', ['competition' => $competition->id, 'sub_competition' => $subCompetition->id]) }}"
    };
...
<!-- Load External JS -->
@vite(['resources/js/admin/sub-competition-skills.js'])
@endcomponent
