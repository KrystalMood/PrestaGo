        ...
        <ul class="space-y-1">
            ...
            <li>
                <a href="{{ route('admin.verification.index') }}" class="flex items-center p-3 rounded-lg {{ request()->routeIs('admin.verification.*') ? 'bg-brand-light bg-opacity-10 text-brand' : 'hover:bg-gray-100 text-gray-700' }}">
                    ...
                    @php
                        $pendingCount = \App\Models\AchievementModel::pending()->count();
                    @endphp
                    @if($pendingCount > 0)
            ...
            <li>
                <a href="{{ route('admin.competitions.index') }}" class="flex items-center p-3 rounded-lg {{ request()->routeIs('admin.competitions.*') ? 'bg-brand-light bg-opacity-10 text-brand' : 'hover:bg-gray-100 text-gray-700' }}">
                    ...
                    @php
                        $pendingRegistrationsCount = \App\Models\SubCompetitionParticipantModel::where('status', 'pending')->count();
                    @endphp
                    @if($pendingRegistrationsCount > 0)
                        <span class="ml-auto inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-red-100 bg-red-600 rounded-full">{{ $pendingRegistrationsCount }}</span>
                    @endif
                </a>
            </li>
            ...
            <li>
                <a href="{{ route('admin.recommendations.automatic') }}" class="flex items-center p-3 rounded-lg {{ request()->routeIs('admin.recommendations.*') ? 'bg-brand-light bg-opacity-10 text-brand' : 'hover:bg-gray-100 text-gray-700' }}">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
