<aside id="mobile-sidebar" class="fixed inset-y-0 left-0 w-64 bg-white shadow-custom z-30 transform -translate-x-full transition-transform duration-300 ease-in-out lg:hidden">
    ...
    <div class="p-4">
        <ul class="space-y-1">
            ...
            <li>
                <a href="{{ route('lecturer.profile.index') }}" class="flex items-center p-3 rounded-lg {{ request()->routeIs('Mahasiswa.settings.*') ? 'bg-brand-light bg-opacity-10 text-brand' : 'hover:bg-gray-100 text-gray-700' }}">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
