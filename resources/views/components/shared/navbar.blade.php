                ...
                <div class="relative ml-4">
                    <div class="dropdown dropdown-end">
                        <div tabindex="0" role="button" class="flex items-center cursor-pointer">
                            <img class="h-8 w-8 rounded-full border-2 border-gray-200" src="https://ui-avatars.com/api/?name={{ $user ? urlencode($user->name) : 'User' }}&background=4338ca&color=fff" alt="User avatar" loading="lazy" />
                        ...
                        <ul tabindex="0" class="dropdown-content menu p-2 mt-1 bg-white border border-gray-200 rounded-lg shadow-custom w-48 z-10">
                            <li class="p-2 text-sm font-medium border-b border-gray-100">
                                <span class="block">{{ $user ? $user->name : (auth()->user() ? auth()->user()->name : 'Unknown User') }}</span>
